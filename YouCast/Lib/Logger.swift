...
private struct Log {
    ...
    static func new(level: Level, message: String) -> Log {
        .init(level: level, message: message, date: .now)
    }
    ...
    func toString() -> String {
        ...
        return "\(emoji) [\(timestamp)] [\(level.rawValue.uppercased())] \(message)\n"
    }
}
...
class Logger {
    ...
    init(
        directory: String = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!.appending("/logs"),
