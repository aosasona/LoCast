...
import { VideoDetails } from "@lib/tauri/types";
	...
	return (
		<Flex direction="column" gap="5">
			<Flex direction="column" align="start" gap="4">
				...
				<Box
					width="auto"
					className="aspect-video object-cover border border-gray-600 rounded-md overflow-hidden"
				>
					<img
				...
				<Flex direction="column" gap="2" className="max-w-full transition-all">
					...
					<Flex gap="2" align="center" wrap="wrap">
						...
						{data.duration_in_seconds && (
							<Badge color="orange">
								{formatDuration(parseInt(data.duration_in_seconds))}
							</Badge>
						)}
						{data.view_count && (
							<Badge color="crimson">
								{parseInt(data.view_count).toLocaleString()} views
							</Badge>
						)}
						{data.publish_date && (
							<Badge color="blue">
					...
					<Text size="2" color="gray">
						{data.description.length > DESCRIPTION_LIMIT
							? `${data.description.slice(0, DESCRIPTION_LIMIT)?.trim()}...`
							: data.description}
					</Text>
				</Flex>
			</Flex>
